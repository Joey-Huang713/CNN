...
class MultiInput(nn.Module):
    ...
    def __init__(self):
        ...
        Laser_Module = list(models.vgg16(pretrained=True).children())[:-1]
        ...
        self.fc = nn.Sequential(nn.Linear(62720,4096),
                                nn.ReLU(),
                                nn.Dropout(p=0.5),
                                nn.Linear(4096,4096),
                                nn.ReLU(),
...
model = torch.load('./model_save/Multi_Input_CNN2.pth')
for param in model.model_Laser.parameters():
    param.requires_grad = False
for param in model.model_FeedRate.parameters():
    param.requires_grad = False
for parma in model.model_Freq.parameters():
    parma.requires_grad = False
path = ".\\CNN_data\\0319Transfer_Data"
...
torch.save(model, './model_save/Transfer1.pth')